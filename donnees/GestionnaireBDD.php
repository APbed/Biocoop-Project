...
class GestionnaireBDD {
    ...
    public function getUneFamille($unIdFamille) {
        ...
        $txtReq = "SELECT id , libelle FROM familleproduit WHERE id = :id";
        $req = $this->_cnx->prepare($txtReq);
        
        $req->bindValue("id", $unIdFamille, PDO::PARAM_INT);
        $resultat = $req->execute();
        if ($resultat === false) {
            $sql = $this->erreurSQL("Pb lors de la recherche de la famille du produit. ", $req);
        }
        $lesProduitFamille = array();
        $uneLigne = $req->fetch(PDO::FETCH_OBJ);
        return $uneLigne;
        /*while($uneLigne != false){
            $uneProduitF = new FamilleProduit($uneLigne->id, $uneLigne->libelle);
            $lesProduitFamille[]= $uneProduitF;
            $uneLigne->fetch(PDO::FETCH_OBJ);
        }
        //$req->closeCursor();
